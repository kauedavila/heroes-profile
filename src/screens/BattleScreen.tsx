...
export const BattleScreen: React.FC<BattleScreenProps> = ({
  ...
  const renderCharacter = (character: BattleCharacter, index: number) => {
    ...
    let imageStyle: ImageStyle | ImageStyle[] = styles.battlerImage;
    if (!isPlayer) {
      imageSource =
        typeof character.image === "string"
          ? monsterImages[character.image] || null
          : null;
      if (character.imageWidth || character.imageHeight) {
        imageStyle = [
          styles.battlerImage,
          character.imageWidth ? { width: character.imageWidth } : {},
          character.imageHeight ? { height: character.imageHeight } : {},
        ];
      }
    } else {
...
const styles = StyleSheet.create({
  ...
  battlerActionMeterFill: {
    height: "100%",
    borderRadius: 5,
    backgroundColor: "#FFD700",
  },
  battlerActionMeterText: {
    fontSize: 11,
    color: "#FFD700",
    fontWeight: "bold",
    marginLeft: 6,
